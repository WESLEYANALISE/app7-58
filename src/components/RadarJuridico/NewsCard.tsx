...
export const NewsCard = ({ news, isRead, isFavorite, onRead, onToggleFavorite }: NewsCardProps) => {
  ...
  return (
    <Card className={`group cursor-pointer transition-all duration-300 hover:shadow-lg hover:scale-[1.02] border-l-4 ${
      isRead ? 'opacity-70 border-l-muted' : 'border-l-yellow-500'
    }`}>
        ...
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex items-center gap-1">
            <Button
              ...
              className={`p-1 ${isFavorite ? 'text-yellow-400 hover:text-yellow-300' : 'text-muted-foreground hover:text-foreground'}`}
            >
      ...
      <CardContent className="pt-0" onClick={() => onRead(news)}>
        <div className="space-y-3">
          ...
          <h3 className={`font-semibold leading-tight line-clamp-3 group-hover:text-yellow-400 transition-colors ${
